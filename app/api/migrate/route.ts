...
/**
 * One-time migration endpoint to enrich players & transfers tables.
...
export async function GET(request: Request) {
  ...
  for (const sql of statements) {
    try {
      ...
      if (error) {
        // Try raw query via pg
        results.push(`⚠️ RPC failed for: ${sql.slice(0, 60)}... — ${error.message}`);
      } else {
        results.push(`✅ ${sql.slice(0, 60)}...`);
      }
    } catch (err) {
      results.push(`❌ ${sql.slice(0, 60)}... — ${String(err)}`);
    }
  }
