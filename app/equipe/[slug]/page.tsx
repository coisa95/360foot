  ...
  return (
    <main className="min-h-screen bg-transparent text-slate-900">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Card className="bg-slate-50 border-slate-200 p-6 mt-6">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-display text-3xl font-bold text-emerald-600">{team.name}</h1>
                  ...
                  {team.coach && (
                    <span className="flex items-center gap-1">
                      ...
                      {team.coach_nationality && (
                        <span className="text-xs text-slate-400">({team.coach_nationality})</span>
                      )}
                    </span>
                  )}
        ...
        {standings && (
          <Card className="bg-slate-50 border-slate-200 p-6 mt-6">
            <h2 className="font-display text-lg font-bold text-emerald-600 mb-4">Position au classement</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-4 text-center">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-600">{standings.position}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-slate-500 text-sm">MJ</p>
                <p className="text-2xl font-bold">{standings.played}</p>
              </div>
              <div>
                <p className="text-slate-500 text-sm">V</p>
                <p className="text-2xl font-bold text-green-600">{standings.won}</p>
              </div>
              <div>
                <p className="text-slate-500 text-sm">N</p>
                <p className="text-2xl font-bold text-slate-500">{standings.drawn}</p>
              </div>
              <div>
                <p className="text-slate-500 text-sm">D</p>
                <p className="text-2xl font-bold text-red-600">{standings.lost}</p>
              </div>
              <div>
                <p className="text-slate-500 text-sm">Diff</p>
        ...
        {/* Team Statistics */}
        {teamStats && (
          <Card className="card-glass border-slate-200 p-6 mt-6">
            <h2 className="font-display text-lg font-bold text-emerald-600 mb-4">📊 Statistiques de la saison</h2>
            <Separator className="bg-slate-200 mb-4" />

            {/* Form */}
            {teamStats.form && (
              <div className="mb-4">
                <p className="text-sm text-slate-500 mb-2">Forme récente</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              {teamStats.goals?.for?.total && (
                <>
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                    <p className="text-xs text-slate-400">Buts marqués</p>
                    <p className="text-2xl font-bold text-emerald-600">{teamStats.goals.for.total.total}</p>
                    <p className="text-[10px] text-slate-400">
                      🏠 {teamStats.goals.for.total.home} | ✈️ {teamStats.goals.for.total.away}
                    </p>
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                    <p className="text-xs text-slate-400">Buts encaissés</p>
                    <p className="text-2xl font-bold text-red-600">{teamStats.goals.against?.total?.total || 0}</p>
                    <p className="text-[10px] text-slate-400">
              ...
              {teamStats.goals?.for?.average && (
                <div className="rounded-lg bg-slate-50 p-3 text-center">
                  <p className="text-xs text-slate-400">Moy. buts/match</p>
                  <p className="text-2xl font-bold text-slate-900">{teamStats.goals.for.average.total}</p>
                </div>
              )}
              ...
              {teamStats.clean_sheet && (
                <div className="rounded-lg bg-slate-50 p-3 text-center">
                  ...
                  <p className="text-2xl font-bold text-green-600">{teamStats.clean_sheet.total}</p>
                </div>
              )}
              ...
              {teamStats.failed_to_score && (
                <div className="rounded-lg bg-slate-50 p-3 text-center">
                  <p className="text-xs text-slate-400">Sans marquer</p>
                  <p className="text-2xl font-bold text-red-600">{teamStats.failed_to_score.total}</p>
                </div>
              )}
              ...
              {teamStats.penalty?.scored && (
                <div className="rounded-lg bg-slate-50 p-3 text-center">
                  ...
                  <p className="text-[10px] text-slate-400">{teamStats.penalty.scored.percentage} réussite</p>
                </div>
              )}
              ...
              {teamStats.biggest?.streak && (
                <div className="rounded-lg bg-slate-50 p-3 text-center">
                  <p className="text-xs text-slate-400">Plus longue série</p>
                  <p className="text-2xl font-bold text-green-600">{teamStats.biggest.streak.wins}V</p>
                  <p className="text-[10px] text-slate-400">{teamStats.biggest.streak.draws}N · {teamStats.biggest.streak.loses}D</p>
                </div>
              )}
            </div>
            ...
            {teamStats.lineups && teamStats.lineups.length > 0 && (
              <div className="mt-4">
                <p className="text-sm text-slate-500 mb-2">Formations utilisées</p>
                <div className="flex flex-wrap gap-2">
                  {teamStats.lineups.map((l: any, i: number) => (
                    <Badge key={i} className="bg-slate-50 text-slate-700 border-slate-300">
                      {l.formation} <span className="ml-1 text-emerald-600">({l.played}x)</span>
                    </Badge>
                  ))}
                </div>
              </div>
        ...
        {upcomingMatches && upcomingMatches.length > 0 && (
          <div className="mt-8">
            <h2 className="font-display text-lg font-bold text-emerald-600 mb-4">Matchs à venir</h2>
        ...
        {recentMatches && recentMatches.length > 0 && (
          <div className="mt-8">
            <h2 className="font-display text-lg font-bold text-emerald-600 mb-4">Derniers résultats</h2>
            ...
            <div className="mt-8">
              ...
              {grouped.map((group) => (
                <div key={group.position} className="mb-6">
                  <h3 className="font-display text-md font-semibold text-slate-700 mb-3">
